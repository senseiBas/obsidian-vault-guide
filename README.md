...
- **Hide folders and files** — set regex patterns (one per line) in the plugin settings to hide matching folders, and any note with a frontmatter property `hidden: true` is hidden too. Toggle the eye button in the navigator to reveal them, greyed out.
